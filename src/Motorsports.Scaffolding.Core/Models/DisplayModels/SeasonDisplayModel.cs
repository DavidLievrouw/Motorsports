...
namespace Motorsports.Scaffolding.Core.Models.DisplayModels {
  public class SeasonDisplayModel {
    ...
    [DisplayFormat(NullDisplayText = "?")]
    public string WinningTeamName => DataModel.RelatedWinningTeam?.RelatedSeasonEntries?.SingleOrDefault(se => se.Season == Id)?.Name ?? DataModel.RelatedWinningTeam?.Name;
